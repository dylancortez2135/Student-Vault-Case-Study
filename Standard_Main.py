...
async def StudentLogging(request: Request, student_number: int = Form(...), password: str = Form(...)):
    ...
    with psycopg.connect(
            ...
            with conn.cursor() as curr:
                ...
                curr.execute("SELECT student_number, year_level, semester, general_weighted_average, academic_honors FROM fct_semestral_grades WHERE student_number = %s", (str(USERNAME),))
                ...
                curr.execute("SELECT descriptive, equivalent_grade, units, remarks, academic_level, semester FROM base_student_grades WHERE student_number = %s", (str(USERNAME),))
